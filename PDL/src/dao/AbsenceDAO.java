...
public class AbsenceDAO extends ConnectionDAO {
	...
	public int addAbsence(Absence absence) {
		...
		PreparedStatement ps = null;
		int returnValue = 0;
		java.util.Date utilDate = new java.util.Date( 2023,  05,  15);
		...
		try {
			...
			// a communiquer dans l'insertion.
			// les getters permettent de recuperer les valeurs des attributs souhaites
			ps = con.prepareStatement("INSERT INTO absence (idabs, idetud, idcours, heuredebut, nbrdheure, date_abs, statut) SELECT ?, ?, ?, ?, ?, ?, ? FROM DUAL WHERE EXISTS (SELECT idetud FROM etudiant WHERE idetud = ?)");

			ps.setInt(1, getList2().get(getList2().size()-1).getId()+1);
			ps.setInt(2, absence.getIdEtud());
			ps.setInt(3, 99);
	...
	public ArrayList<Absence> getList2() {
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		ArrayList<Absence> returnValue = new ArrayList<Absence>();
